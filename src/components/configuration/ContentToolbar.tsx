...
const BTN =
  'flex cursor-pointer items-center gap-1 rounded border-none bg-transparent px-1.5 py-0.5 text-xs text-(--ui-color-text-muted) transition-colors hover:bg-(--ui-color-background-hover) hover:text-(--ui-color-text-default)';
...
export function ContentToolbar({
  ...
  return (
    <div className="pointer-events-auto mt-2 mr-3 inline-flex items-center rounded-lg border border-(--ui-color-stroke-default) bg-(--ui-color-background-panel)/90 shadow-sm backdrop-blur-sm">
      {collapsed ? (
        <button
          type="button"
          onClick={() => setCollapsed(false)}
          title={localize('com_config_show_toolbar')}
          className="flex cursor-pointer items-center border-none bg-transparent px-2.5 py-1.5 text-(--ui-color-text-muted) transition-colors hover:text-(--ui-color-text-default)"
        >
        ...
        <div className="flex items-center gap-1 px-1.5 py-1">
          ...
          <span className="text-xs text-(--ui-color-text-disabled)">/</span>
          <button
            type="button"
          ...
          {showConfiguredToggle && (
            <>
              <span className="mx-0.5 h-3.5 w-px bg-(--ui-color-stroke-default)" />
              <label
                className="flex cursor-pointer items-center gap-1.5 rounded px-1.5 py-0.5 text-xs text-(--ui-color-text-muted) transition-colors select-none hover:bg-(--ui-color-background-hover) has-focus-visible:outline-1 has-focus-visible:outline-(--ui-color-outline)"
                onKeyDown={(e) => {
              ...
              >
                <input
                  ...
                  className="accent-(--ui-color-accent) focus-visible:outline-none"
                />
