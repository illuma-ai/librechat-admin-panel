...
export function ScopeSelector({
    ...
    return (
      ...
      >
        ...
        <div className="flex flex-col gap-4 p-4">
          <p className="text-sm text-(--ui-color-text-default)">
          ...
          <div className="flex justify-end gap-2">
            <button
              ...
              disabled={deleting}
              className="cursor-pointer rounded-md px-3 py-1.5 text-sm text-(--ui-color-text-muted) transition-colors hover:text-(--ui-color-text-default)"
            >
  ...
  if (showCreate) {
    ...
    return (
      ...
      >
        ...
        <div className="flex items-center gap-2 border-b border-(--ui-color-stroke-default) px-4 py-3">
          <button
            type="button"
            onClick={() => setShowCreate(false)}
            className="flex cursor-pointer items-center text-(--ui-color-text-muted) hover:text-(--ui-color-text-default)"
          >
        ...
        <div ref={listRef} className="max-h-95 overflow-y-auto p-2 pb-3">
          ...
          <div className="cmdk-group">
            ...
            {noRoles ? (
              <p className="px-4 py-4 text-center text-xs text-(--ui-color-text-muted)">
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 border-b border-(--ui-color-stroke-default) px-4">
        <Command.Input
          value={search}
          onValueChange={handleSearchChange}
          placeholder={localize('com_scope_search')}
          className="flex min-w-0 flex-1 bg-transparent py-3 text-sm text-(--ui-color-text-default) outline-none placeholder:text-(--ui-color-text-muted)"
        />
        {canAssign && (
          <Button
            type="secondary"
            ...
            <Command.Group heading={localize('com_scope_base_config')} className="cmdk-group">
              ...
              >
                ...
                {currentSelection.type === 'BASE' && (
                  <span aria-hidden="true" className="shrink-0 text-(--ui-color-accent)">
    ...
    >
      ...
      <span className="flex min-w-0 flex-1 flex-col">
        <span className="flex items-center gap-2">
          <span className="text-sm font-medium text-(--ui-color-text-default)">{scope.name}</span>
          {!scope.isActive && (
            <span className="rounded-sm bg-(--ui-color-background-secondary) px-1.5 py-0.5 text-[10px] font-medium text-(--ui-color-text-muted)">
              {localize('com_scope_inactive')}
            </span>
          )}
        </span>
        {memberText && <span className="text-xs text-(--ui-color-text-muted)">{memberText}</span>}
      </span>
      {isSelected && (
        <span aria-hidden="true" className="shrink-0 text-(--ui-color-accent)">
          <Icon name="check" size="sm" />
      ...
      {onDelete && (
        <button
          ...
          className="shrink-0 cursor-pointer rounded-sm p-0.5 text-(--ui-color-text-muted) opacity-0 transition-opacity group-hover:opacity-100 group-data-[active=true]:opacity-100 group-data-[selected=true]:opacity-100 hover:text-(--ui-color-accent-danger)"
...
export function ScopeTriggerButton({ currentSelection, onClick }: t.ScopeTriggerButtonProps) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex shrink-0 cursor-pointer items-center gap-1.5 rounded-lg border border-(--ui-color-stroke-default) bg-transparent px-3 py-1.5 text-sm text-(--ui-color-text-default) transition-colors hover:bg-(--ui-color-background-hover)"
    ...
    >
      ...
      <span>{label}</span>
      <span aria-hidden="true" className="text-(--ui-color-text-muted)">
