    ...
    <Dropdown>
      <Dropdown.Trigger>
        <button
          ...
          className="inline-flex h-8 w-30 cursor-pointer items-center justify-between gap-1 rounded-l-none rounded-r-md border border-l-0 border-(--ui-color-stroke-default) px-2 text-sm text-(--ui-color-text-default) hover:bg-(--ui-color-background-muted)"
        >
          ...
          <Dropdown.Item key={option.value} onClick={() => onChange(option.value)}>
            <span
              className={cn(
                ...
                value === option.value && 'font-medium text-(--ui-color-text-default)',
              )}
            >
