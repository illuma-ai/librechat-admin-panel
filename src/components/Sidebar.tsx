            ...
            <div className="flex items-center gap-2.5 overflow-hidden px-0.5">
                  ...
                  <div className="user-dropdown flex min-w-45 flex-col select-none">
                    {user && (
                      <div className="px-3 pt-3 pb-2">
                        <span className="block text-sm leading-tight font-medium text-(--ui-color-text-default)">
                          {user.name || ''}
                        </span>
                        {user.email && (
                          <span className="mt-0.5 block text-xs leading-tight text-(--ui-color-text-muted)">
                            {user.email}
                          </span>
                        )}
                      </div>
                    )}
                    <div className="border-t border-(--ui-color-stroke-default)" />
              ...
              {user && (
                <div className="min-w-0 flex-1">
                  <span className="block truncate text-sm leading-tight font-medium text-(--ui-color-text-default)">
                    {user.name || ''}
                  </span>
                  {user.email && (
                    <span className="block truncate text-xs leading-tight text-(--ui-color-text-muted)">
                      {user.email}
                    </span>
                  )}
                </div>
